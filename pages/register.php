...
die("Registrations from proxies aren't allowed. Turn off your proxy and try again. If you're not using a proxy, contact mathmaster01@gmail.com"); 
?>

<?php

if($loguserid)
	Kill(__("You can't register while logged in."));
...
require_once(BOARD_ROOT.'lib/ayah/ayah.php');
$ayah = new AYAH();
...
if (array_key_exists('register', $_POST))
{
        ...
        {
                echo "Sorry, but we were not able to verify you as a human. Please try again.";
        }
}
...
if($_POST['register'])
{
	...
	else
	{
		...
		elseif($ipKnown >= 1)
			$err = __("No duplicate accounts allowed. If you lost your password, contact one of the staff members by email or on IRC.");
		...
		else if (preg_match("@^(MKDS|MK7|SM64DS|SMG|NSMB|StapleButter|blarg|board|Pizza)\d*?@si", $uname))
