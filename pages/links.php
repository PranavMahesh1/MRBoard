...
if(HasPermission('admin.editsettings'))
	$links[] = actionLinkTag(__("Edit the links list"), "editsettings", '', 'field=linksText');

MakeCrumbs(array(actionLink("links") => __("Links")), $links);
...
$sites = Settings::get("linksText");

$sites = str_replace("<colortable />", $colortable, $sites);
if("" == Settings::get("registrationWord"))
	$sites = preg_replace("'<iftheword>(.*)</iftheword>'s", "", $sites);
else
	$sites = str_replace("<theword />", Settings::get("registrationWord"), $sites);
...
$sites = str_replace("<themeexample1 />", DoGeshi($code1), $sites);
$sites = str_replace("<themeexample2 />", DoGeshi($code2), $sites);
$sites = str_replace("<themelist />", $finaltlist, $sites);
$sites = str_replace("<admin />", $admin, $sites);
