...
$temp = array();
                        ...
                        if($posts >= 65) $level = 7;
                        if($posts >= 80) $level = 8;
                        if($posts >= 100) $level = 9;
                        if($posts >= 120) $level = 10;
                        if($posts >= 140) $level = 11;
                        if($posts >= 160) $level = 12;
                        if($posts >= 180) $level = 13;
                        if($posts >= 200) $level = 14;
                        if($posts >= 225) $level = 15;
                        if($posts >= 250) $level = 16;
                        if($posts >= 275) $level = 17;
                        if($posts >= 300) $level = 18;
                        if($posts >= 325) $level = 19;
                        if($posts >= 350) $level = 20;
                        if($posts >= 375) $level = 21;
                        if($posts >= 400) $level = 22;
                        if($posts >= 425) $level = 23;
                        if($posts >= 450) $level = 24;
                        if($posts >= 475) $level = 25;
                        if($posts >= 500) $level = 26;
                        if($posts >= 525) $level = 27;
                        if($posts >= 550) $level = 28;
                        if($posts >= 575) $level = 29;
                        if($posts >= 600) $level = 30;
                        if($posts >= 625) $level = 31;
                        if($posts >= 650) $level = 32;
                        if($posts >= 675) $level = 33;
                        if($posts >= 700) $level = 34;
                        if($posts >= 725) $level = 35;
                        if($posts >= 750) $level = 36;
                        if($posts >= 775) $level = 37;
                        if($posts >= 800) $level = 38;
                        if($posts >= 830) $level = 39;
                        if($posts >= 860) $level = 40;
                        if($posts >= 900) $level = 41;
                        if($posts >= 925) $level = 42;
                        if($posts >= 950) $level = 43;
                        if($posts >= 1000) $level = 44;
                        if($posts >= 1050) $level = 45;
                        if($posts >= 1100) $level = 46;
                        if($posts >= 1150) $level = 47;
                        if($posts >= 1200) $level = 48;
                        if($posts >= 1250) $level = 49;
                        if($posts >= 1300) $level = 50;
                        if($posts >= 1350) $level = 51;
                        if($posts >= 1400) $level = 52;
                        if($posts >= 1450) $level = 53;
                        if($posts >= 1500) $level = 54;
                        if($posts >= 1550) $level = 55;
                        if($posts >= 1600) $level = 56;
                        if($posts >= 1650) $level = 57;
                        if($posts >= 1700) $level = 58;
                        if($posts >= 1800) $level = 59;
                        if($posts >= 1900) $level = 60;
                        if($posts >= 2000) $level = 61;
                        if($posts >= 2100) $level = 62;
                        if($posts >= 2200) $level = 63;
                        if($posts >= 2300) $level = 64;
                        if($posts >= 2400) $level = 65;
                        if($posts >= 2500) $level = 66;
                        if($posts >= 2600) $level = 67;
                        if($posts >= 2700) $level = 68;
                        if($posts >= 2800) $level = 69;
                        if($posts >= 2900) $level = 70;
                        if($posts >= 3000) $level = 71;
                        if($posts >= 3100) $level = 72;
                        if($posts >= 3200) $level = 73;
                        if($posts >= 3300) $level = 74;
                        if($posts >= 3400) $level = 75;
                        if($posts >= 3500) $level = 76;
                        if($posts >= 3600) $level = 77;
                        if($posts >= 3700) $level = 78;
                        if($posts >= 3800) $level = 79;
                        if($posts >= 3900) $level = 80;
                        if($posts >= 4000) $level = 81;
                        if($posts >= 4100) $level = 82;
                        if($posts >= 4200) $level = 83;
                        if($posts >= 4300) $level = 84;
                        if($posts >= 4400) $level = 85;
                        if($posts >= 4500) $level = 86;
                        if($posts >= 4600) $level = 87;
                        if($posts >= 4700) $level = 88;
                        if($posts >= 4800) $level = 89;
                        if($posts >= 4900) $level = 90;
                        if($posts >= 5000) $level = 'Maxed out';
                        
                        //Coins and Lives
                        $coins = 0 + (4 * $user['postplusones']) - (5 * $deletedposts) + $posts;
                        if($coins < 0) {
							$coins = 0;
						}
						$lives = round($coins/10);
                        //EXP Stuff
                        $expnum = 127;
                        $exp = $expnum + ($posts * 135);
						
                        //Value stuff
                        $currenthp = (25 + $posts) - (3 * $deletedposts) + (2 * $user['postplusones']);
                        $hp = (25 + $posts);
						if($currenthp > $hp) {
							$currenthp = (25 + $posts);
						}
                        ...
                        $atk = (round($posts/4));
                        $def = (round($posts/5));
                        $iq = $threads + (1 * $user['postplusones']);
                        $spr = (round($posts/5.5)); 
                        $lck = (round($threads/2)) + (round($posts/5));
                        $agi = (round(($posts+$threads)/6));
                        
                        //RPG Table
                        $temp[__("Level")] = $level;
                        $temp[__("HP")] = "$currenthp/$hp";
                        $temp[__("MP")] = "$mp/$mp";
                        $temp[__("Pow")] = $atk;
                        $temp[__("Def")] = $def;
                        $temp[__("IQ")] = $iq;
...
if(count($temp))
	$profileParts[__("RPG Stats")] = $temp;
...
if(HasPermission('admin.usernuke'))
	$links[] = actionLinkTag(__('NUKE USER'), 'nuke', '', 'id='.$id);
if(HasPermission('admin.pmnuke'))
	$links[] = actionLinkTag(__('PM NUKE'), 'pmnuke', '', 'id='.$id);
if(HasPermission('admin.commentnuke'))
	$links[] = actionLinkTag(__('COMMENT NUKE'), 'profilecommentsnuke', '', 'id='.$id);
if(HasPermission('admin.manageipbans'))
	$links[] = actionLinkTag(__('IP Ban'), 'instaipban', ''.$id);
