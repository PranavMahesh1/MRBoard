...
function makePost($post, $type, $params=array())
{
	...
	$coins = (15 * $poster['postplusones']) - (10 * $deletedposts) + (1 * $posts);
	$lives = $coins / 100;
	if($poster['title'])
	...
	if($post['mood'] > 0)
	{
		if(file_exists("data/avatars/".$poster['id']."_".$post['mood']))
			$sidebar['avatar'] = "<img src=\"/data/avatars/".$poster['id']."_".$post['mood']."\" id=\"avatar".$poster['id']."\" alt=\"\">";
	}
