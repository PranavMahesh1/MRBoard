...
function makeNewsBar()
{
	...
	$newsforum = Settings::get('newsForum');
	if ($newsforum > 0)
	{
		$news = Query("	SELECT 
							t.id, t.title, t.icon, t.poll, t.forum,
						...
						FROM 
							...
							LEFT JOIN {users} u ON u.id=t.user
						WHERE forum={0}
						ORDER BY newsdate DESC LIMIT 1", $newsforum, $loguserid);
								
		if ($news && NumRows($news))
		{
			$news = Fetch($news);
			$adata = array();
			
			$adata['new'] = '';
			if ((!$loguserid && $news['newsdate'] > (time()-900)) ||
				($loguserid && $news['newsdate'] > $news['readdate']))
				$adata['new'] = "<div class=\"statusIcon new\"></div>";
			
			$adata['poll'] = ($news['poll'] ? "<img src=\"".resourceLink('img/poll.png')."\" alt=\"Poll\"/> " : '');
			$adata['link'] = MakeThreadLink($news);
			
			$user = getDataPrefix($news, 'u_');
			$adata['user'] = UserLink($user);
			$adata['date'] = formatdate($news['newsdate']);
			
			RenderTemplate('newsbar', array('news' => $adata));
		}
	}
}
?>
